...
int main() {
    ...
    hsa_executable_create(HSA_PROFILE_FULL, HSA_EXECUTABLE_STATE_UNFROZEN, nullptr, &executable);
    ...
    std::cout << "Kernel object: " << kernel_object << "\n";

    // ---- Backing pool selection ----
    ...
    std::string contents((std::istreambuf_iterator<char>(meta)), std::istreambuf_iterator<char>());
    ...
    while ((pos = contents.find("\"base\":", pos)) != std::string::npos) {
        ...
        if (st!=HSA_STATUS_SUCCESS) { std::cerr<<"reserve failed\n"; return 1; }
        ...
        st = hsa_amd_vmem_handle_create(
                ...
                0,
                &handle);
        if (st!=HSA_STATUS_SUCCESS){ std::cerr<<"handle failed\n"; return 1; }

        st = hsa_amd_vmem_map(reserved,size,0,handle,0);
        if (st!=HSA_STATUS_SUCCESS){ std::cerr<<"map failed\n"; return 1; }
        ...
        if (st!=HSA_STATUS_SUCCESS){ std::cerr<<"access failed\n"; return 1; }

        std::stringstream fname;
        fname<<"../../isolate/tool/isolate_capture/memory/region_"<<std::hex<<base<<".bin";
        std::ifstream blobf(fname.str(),std::ios::binary);
        std::vector<char> blob((std::istreambuf_iterator<char>(blobf)),std::istreambuf_iterator<char>());

        st = hsa_memory_copy(reserved,blob.data(),size);
        if (st!=HSA_STATUS_SUCCESS){ std::cerr<<"copy failed\n"; return 1; }
    ...
    std::vector<char> kblob((std::istreambuf_iterator<char>(kf)),std::istreambuf_iterator<char>());
    memcpy(kernarg,kblob.data(),kblob.size());

    // ---- Create queue ----
    hsa_queue_t* queue=nullptr;
    hsa_queue_create(g_gpu_agent,128,HSA_QUEUE_TYPE_MULTI,nullptr,nullptr,0,0,&queue);

    const uint64_t idx = hsa_queue_load_write_index_relaxed(queue);
    hsa_kernel_dispatch_packet_t* pkt =
        reinterpret_cast<hsa_kernel_dispatch_packet_t*>(queue->base_address)+idx;
