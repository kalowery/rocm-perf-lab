...
def test_pointer_chase_isolate_and_replay(tmp_path):
    repo_root = Path(__file__).resolve().parents[2]
    kernel_src = repo_root / "tests" / "integration" / "pointer_chase_kernel.cpp"

    # Build pointer chase kernel
    binary = tmp_path / "pointer_chase_kernel"
    run(["hipcc", str(kernel_src), "-o", str(binary)])

    # Build path to isolate shared library
    ...
    env["ISOLATE_KERNEL"] = "pointer_chase"
